namespace BenchmarkingApp.Benchmarks.Data {
    ...
    public static class StringGenerator {
        ...
        static readonly IDictionary<int, string> ipsumsCache = new Dictionary<int, string>();
        public static string LoremIpsum(this Random random) {
            int length = random.Next(loremipsum.Length / 2, loremipsum.Length);
            string ipsum;
            if(!ipsumsCache.TryGetValue(length, out ipsum)) {
                string[] words = new string[length];
                for(int i = 0; i < words.Length; i++) {
                    words[i] = loremipsumWords[random.Next(loremipsumWords.Length)];
                }
                ipsum = string.Join(" ", words);
                ipsumsCache.Add(length, ipsum);
            }
            return ipsum;
        }
    }
